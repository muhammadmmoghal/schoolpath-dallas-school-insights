...
render_page_header(
    "Special Education",
    subtitle="Special-ed membership, CRDC disability discipline, restraint, and harassment counts.",
)

render_note_card(
    heading="Two data sources — two different years",
    content=(
...
with st.container(border=True):
    render_card_header("Key Metrics", caption="TAPR 2025 | CRDC 2021-22")
...
with st.container(border=True):
    render_card_header(
        "Special Education Membership % (TAPR 2025)",
        caption="Denominator: all-student membership. TEA masking codes (-1, blank) converted to null.",
    )

    null_note(sped_col, df[sped_col].isna().sum(), n, friendly_name="Special Ed Membership %")
    ...
    with col2:
        ...
        level_display_df["school_level"] = (
            level_display_df["school_level"].map(LEVEL_LABELS)
            .fillna(level_display_df["school_level"])
        )
        level_cat_order = [
            LEVEL_LABELS.get(l, l)
            for l in LEVEL_ORDER
            if LEVEL_LABELS.get(l, l) in level_display_df["school_level"].values
        ]
        box_plot(
            level_display_df,
            x="school_level",
    ...
    if pd.notna(med_sped):
        ...
        st.caption(
            ...
            "exceed 20% special-ed membership. "
...
with st.container(border=True):
    render_card_header(
        "Attendance Disparity: All Students vs. Special Education (TAPR 2024)",
        caption=(
    ...
    if not gap_df.empty:
        ...
        fig_gap.add_vline(x=0, line_dash="dash", line_color="#CBD5E1")
...
with st.container(border=True):
    render_card_header(
        "CRDC: IDEA Enrollment Count (2021-22)",
        caption="Raw enrollment counts — not comparable to current-year TAPR rates. Zero is a real reported zero.",
    )
...
with st.container(border=True):
    render_card_header(
        "CRDC: Disability-Related Discipline Counts (2021-22)",
        caption="Raw counts, not rates. Schools vary in size. Null ≠ zero.",
    )
    st.warning(
...
with st.container(border=True):
    render_card_header(
        "CRDC: Restraint and Seclusion — IDEA Students (2021-22)",
        caption="Instance counts (not student counts). Null = not reported or not applicable. Zero = reported zero.",
    )
...
with st.container(border=True):
    render_card_header(
        "CRDC: Harassment and Bullying — Disability Basis (2021-22)",
        caption=(
            "'Allegations' = formal allegations. "
            "'Students reported' = reported as harassed. "
            "'Disciplined' = alleged harassers disciplined. "
            "Null = not reported. Zero = reported zero."
        ),
    )
