...
render_page_header(
    "School Detail",
    subtitle="Complete data profile for one school — all fields, source years, and documented gaps.",
)
...
with st.container(border=True):
    render_card_header("School Identity")
...
with st.container(border=True):
    render_card_header("Enrollment & Accountability")
    ...
    with col7:
        if pd.notna(rating):
            st.markdown(
                f"<h2 style='color:{rating_color};margin:0;font-family:Sora,Inter,sans-serif;"
                f"font-weight:700;font-size:2.5rem;letter-spacing:-0.03em'>{rating}</h2>",
...
with st.container(border=True):
    render_card_header("Special Education")
    ...
    if not crdc_matched:
        ...
        for col_name, label in SPED_DISC.items():
            ...
            display_val = f"{int(val):,}" if pd.notna(val) else "— (not reported)"
...
# ── Section 4: Attendance & Chronic Absence ───────────────────────────────────

with st.container(border=True):
    render_card_header(
        "Attendance & Chronic Absence (TAPR 2024)",
        caption="Source: TEA TAPR 2025 release; attendance measures are for year 2024.",
    )
...
with st.container(border=True):
    render_card_header("Teacher Staffing (TAPR 2025)", caption="Source: TEA TAPR 2025.")
    ...
    for col_name, lbl in EXP_BANDS:
        val = row.get(col_name)
        display_val = f"{float(val):.1f}%" if pd.notna(val) else "— (not reported)"
...
with st.container(border=True):
    render_card_header(
        "Data Coverage",
        caption="Source-match rates are cohort-wide (all 60 schools), not specific to this school.",
    )
...
with st.container(border=True):
    render_card_header(
        "Missing Data for This School",
        caption="Null means unreported, suppressed, or not applicable — not zero.",
    )
