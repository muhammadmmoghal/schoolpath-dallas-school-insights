...
render_page_header(
    "School Explorer",
    subtitle="Filter, map, and compare all 60 schools in the Dallas cohort.",
)
...
with st.container(border=True):
    render_card_header(
        "Dallas School Map",
        caption="Color = TEA 2025 accountability rating. Coordinates: TEA ArcGIS 2024-25 (geocoder output, display only).",
    )
    scatter_map(
        filtered,
        title=None,
...
with st.container(border=True):
    render_card_header(
        ...
        caption="Sources: AskTED 2025 | TAPR 2025 | TEA Accountability 2025",
    )
    ...
    st.dataframe(display, column_config=col_cfg, use_container_width=True, hide_index=True)
